...
export function createCareerHqMcpServer(bridge: BridgeClient): McpServer {
  ...
  stage("propose_profile_update", "Stage structured identity/contact fields for local review.", { external_id: z.string().trim().min(1).max(300), canonical_key: z.string().trim().min(1).max(500).default("identity:primary-profile"), name: z.string().trim().min(1).max(500), primary_email: z.string().email().optional(), phone: z.string().trim().max(100).optional(), portfolio_url: z.string().trim().max(2_000).optional(), source: z.object(source).strict() }, (a) => ({ operation: "propose_profile_update", ...a }));
  stage("propose_education", "Stage a structured education record for local review.", { external_id: z.string().trim().min(1).max(300), canonical_key: z.string().trim().min(1).max(500), institution: z.string().trim().min(1).max(500), degree: z.string().trim().min(1).max(500), major: z.string().trim().max(500).optional(), concentration: z.string().trim().max(500).optional(), graduation_date: z.string().trim().max(100).optional(), gpa: z.string().trim().max(50).optional(), source: z.object(source).strict() }, (a) => ({ operation: "propose_education", ...a }));
  stage("propose_experience", "Stage a structured employment record for local review.", { external_id: z.string().trim().min(1).max(300), canonical_key: z.string().trim().min(1).max(500), employer: z.string().trim().min(1).max(500), title: z.string().trim().min(1).max(500), location: z.string().trim().max(500).optional(), start_date: z.string().trim().max(100).optional(), end_date: z.string().trim().max(100).optional(), is_current: z.boolean().default(false), responsibilities: z.array(z.string().trim().min(1).max(2_000)).max(100).default([]), source: z.object(source).strict() }, (a) => ({ operation: "propose_experience", ...a }));
  stage("propose_project", "Stage a structured project record for local review.", { external_id: z.string().trim().min(1).max(300), canonical_key: z.string().trim().min(1).max(500), project_name: z.string().trim().min(1).max(500), project_status: z.string().trim().max(200).optional(), project_url: z.string().trim().max(2_000).optional(), technologies: z.array(z.string().trim().min(1).max(200)).max(100).default([]), summary: z.string().trim().max(10_000).optional(), source: z.object(source).strict() }, (a) => ({ operation: "propose_project", ...a }));
  stage("propose_skills", "Stage a structured set of skills for local review.", { external_id: z.string().trim().min(1).max(300), canonical_key: z.string().trim().min(1).max(500).default("skills:proposed-set"), skills: z.array(z.string().trim().min(1).max(200)).min(1).max(100), source: z.object(source).strict() }, (a) => ({ operation: "propose_skills", ...a }));
